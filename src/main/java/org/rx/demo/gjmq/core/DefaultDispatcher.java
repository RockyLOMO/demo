...
public class DefaultDispatcher<T extends QueueElement> extends Disposable implements Dispatcher<T>, EventPublisher<DefaultDispatcher<T>> {
    @Data
    ...
    public final Delegate<DefaultDispatcher<T>, DiscardEventArgs<T>> onDiscard = Delegate.<DefaultDispatcher<T>, DiscardEventArgs<T>>create()
            .first((s, e) -> e.element.setStatus(QueueElementStatus.DETACHED));
    @Getter
...
//            log.debug("element invalid: {}", element);
            try {
                FluentWait.polling(maxWaitInvalidMillis).await(s -> isElementValid(element));
...
//                    log.debug("reValid check: {} | {}", nd, sum);
        ...
        while ((consumer = selector.select(group, element)) != null) {
            ...
            if (!handler.accept(consumerId, element)) {
                ...
                FluentWait.polling(maxAcceptMillis).awaitTrue(s -> handler.accept(consumerId, element));
            }
