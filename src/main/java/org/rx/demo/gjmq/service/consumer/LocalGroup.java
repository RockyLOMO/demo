...
public class LocalGroup<T extends QueueElement> implements ConsumerGroup<T> {
    ...
    final Map<Long, org.rx.demo.gjmq.core.Queue<T>> queues = new ConcurrentHashMap<>();
    @Setter
    ...
    public long computeId(long consumerId) {
        return CodecUtil.hash64(groupId, String.valueOf(consumerId));
    }

    long computeTagId(long consumerId, Tag tag) {
        require(tag, !Strings.isEmpty(tag.getName()));

        return CodecUtil.hash64(groupId, String.valueOf(consumerId), tag.getName(), tag.getValue());
    }
    ...
    @Override
    public org.rx.demo.gjmq.core.Queue<T> getConsumerQueue(long consumerId) {
    ...
    public List<Consumer<T>> nextList(int size) {
        ...
        return Linq.from(r).select(p -> p.content).toList();
    }

    @Override
    public Set<Tag> getTagCapacities() {
        //todo groupby
        return Linq.from(db.findBy(queryTag())).select(p -> new Tag(p.getName(), p.getValue())).toSet();
    }

    @Override
    public List<Consumer<T>> findByTags(List<Tag> tags) {
        return db.transInvoke(Connection.TRANSACTION_READ_COMMITTED, () -> {
            List<TagEntity> r = db.findBy(queryTag().in(TagEntity::getName, Linq.from(tags).select(Tag::getName).toArray()));
            if (CollectionUtils.isEmpty(r)) {
                return Collections.emptyList();
            }
            Long[] cIds = Linq.from(r).join(tags, (p, x) -> eq(p.value, x.getValue()), (p, x) -> p.consumerId).distinct().toArray();
            if (cIds.length == 0) {
                return Collections.emptyList();
            }
            return Linq.from(db.findBy(query().in(ConsumerEntity::getId, cIds))).select(p -> p.content).toList();
        });
    }
}
